                            ...
                            <v-col cols="12" md="12" style="padding: 10px" v-if="Space.addSpaceInput == true">
                              <v-row style="align-items: center;">
                                ...
                                <v-col cols="12" md="2" style="padding: 10px">
                                  <v-btn :id="'addHouseFloorBtn-' + index" @click="addHouesFloor(index)" color="primary"
                                    >ادخال</v-btn
                                  >
                                </v-col>
                                              ...
                                              >
                                                ...
                                                >
                                                  <img
                                                    ...
                                                    :src="isBase64(room.image) ? room.image : content_url + room.image"
                                                    alt="Image"
                                                    @click.stop
                                                  />
                                                  <v-btn
...
export default {
  ...
  methods: {
    ...
    adCopyD(index) {
      ...
      if(this.data.Spaces[index].copyD.building_space !== null && this.data.Spaces[index].copyD.total_space !== null) {
        for (var i = 0; i < this.data.Spaces.length; i++) {
          if(this.data.Spaces[i].building_space == this.data.Spaces[index].copyD.building_space && this.data.Spaces[i].total_space == this.data.Spaces[index].copyD.total_space){
            this.data.Spaces[index].rooms = JSON.parse(JSON.stringify(this.data.Spaces[i].rooms));
            break;
          }
        }
        this.data.Spaces[index].copyD = null;
      } else {
    ...
    async addCenter() {
      ...
      var BuildingNames = []
      for (let j = 0; j < this.tagsBuildingNames.length; j++) {
        BuildingNames.push(this.tagsBuildingNames[j].text);
      }
      try {
        const response = await API.editApartment({
          id: this.data.id,
          name: this.data.name,
          ...
          apartment_building: BuildingNames,
    ...
    async getHouse() {
      try {
        ...
        this.data.images = response.data.results.images;
        this.tagsBuildingNames = [];
        response.data.results.apartment_building.forEach((tag) => {
        ...
        for (var i = 0; i < spaces.length; i++) {
          ...
          var groupedHouses = space.houses.reduce((acc, house) => {
              const floorNumber = house.apartment_floor_number;
              if (!acc[floorNumber]) {
                  acc[floorNumber] = [];
              }
              acc[floorNumber].push(house.name);
              return acc;
          }, {});

          var result = Object.keys(groupedHouses).map(floorNumber => ({
              HouseNumber: groupedHouses[floorNumber],
              FloorNumber: floorNumber
          }));

          this.data.Spaces[i].table = result;

          Object.keys(groupedHouses).forEach(floorNumber => {
              this.data.houses.push({
                  names: groupedHouses[floorNumber],
                  total_space: this.data.Spaces[i].total_space,
                  building_space: this.data.Spaces[i].building_space,
                  apartment_floor_number: floorNumber,
              });
          });
