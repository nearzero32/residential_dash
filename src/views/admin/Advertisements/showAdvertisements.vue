...
export default {
  ...
  methods: {
    ...
    async getCenter(newOptions) {
      ...
      try {
        const response = await adminApi.getPostings({
          page,
          limit: itemsPerPage,
    ...
    async addCenter() {
      ...
      if (valid) {
        ...
        try {
          const response = await adminApi.addPostings({
    ...
    async editItemConform() {
      ...
      if (valid) {
        ...
        try {
          const response = await adminApi.editPostings({
            postings_id: this.dialogEdit.editedItem._id,
