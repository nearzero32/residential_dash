      ...
      <div class="mt-4">
        <v-card>
          <v-card-title>
            ...
            <v-spacer></v-spacer>
            <v-text-field v-model="table.search" @input="getCenter" append-icon="mdi-magnify" label="بحث" single-line
  ...
  data() {
    return {
      ...
      Rules: {
        ...
        description: [(v) => !!v || "يرجى أضافة تفاصيل"],
...
if (userDataString.type !== "admin") {
  ...
  methods: {
    ...
    async addCenter(event) {
      ...
      try {
        const response = await API.addPostings({
    ...
    async editItemConfirm(event) {
      ...
      try {
        const response = await API.editPostings({
          postings_id: this.editdItem._id,
    ...
    async deleteItemConfirm() {
      ...
      try {
        const response = await API.removePostings(this.deletedItem._id);
