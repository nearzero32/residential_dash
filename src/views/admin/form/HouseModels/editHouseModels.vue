...
export default {
  ...
  data() {
    return {
      ...
      getItem: JSON.parse(localStorage.getItem("editForm")),
      id: JSON.parse(localStorage.getItem("editForm"))._id,
      files: [],
      visibleDeleteIcons: [],
      data: {
        name: null,
  ...
  methods: {
    ...
    async getCenter() {
      try {
        const response = await adminApi.getDataHouse(this.id);

        this.data.name = response.data.results.name;
        this.data.images = response.data.results.images;
        this.data.floors = response.data.results.floors;
        this.data.Spaces = response.data.results.spaces.map((space) => {
          return {
            addSpaceInput: true,
            isStorg: false,
            total_space: space.total_space.toString(),
            building_space: space.building_space.toString(),
            tag: {
              houseNumber: "",
              tagsHouseNumber: [],
              category: null,
