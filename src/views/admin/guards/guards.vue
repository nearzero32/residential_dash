<template>
  <v-container fluid class="down-top-padding">
        ...
        <v-card>
          <v-card-title>
            <v-btn
              ...
              @click="dialog = true"
            >
              <v-icon class="mr-2">mdi-plus</v-icon>اٍضافة حارس جديد
            </v-btn>
    ...
    <v-dialog v-model="dialog" max-width="800px">
      <v-card>
        <v-card>
          <v-card-title class="text-h5">اٍضافة حارس جديد</v-card-title>
          <v-divider></v-divider>
          <!----Account Details---->
...
export default {
  ...
  methods: {
    Print(item) {
      ...
      window.open("/Print-Guards", "_blank");
    },
    ...
    async getCenter() {
      try {
        ...
        const response = await API.getGuards({
          page,
          limit: itemsPerPage,
          sortBy: sortByJSON,
    ...
    async addCenter(event) {
      ...
      try {
        const response = await API.addGuards({
          name: this.data.name,
    ...
    async editItemConfirm(event) {
      ...
      try {
        const response = await API.editGuards({
    ...
    async deleteItemConfirm() {
      ...
      try {
        const response = await API.removeGuards(this.deletedItem._id);
