...
export default {
  data() {
    return {
      ...
      action: [
        { text: "محاسب", value: "محاسب" },
        { text: "صيانة", value: "صيانة" },
        { text: "استعلامات", value: "استعلامات" },
        { text: "مهندس", value: "مهندس" },
...
if (userDataString.type !== "admin") {
  ...
  methods: {
    ...
    async addCenter(event) {
      ...
      try {
        const response = await API.addEmployees({
          name: this.data.name,
    ...
    async editItemConfirm(event) {
      ...
      try {
        const response = await API.editEmployees({
          emp_id: this.editdItem._id,
          name: this.editdItem.name,
          title_jop: this.editdItem.title_jop,
    ...
    async deleteItemConfirm() {
      ...
      try {
        const response = await API.removeEmployees(this.deletedItem._id);
