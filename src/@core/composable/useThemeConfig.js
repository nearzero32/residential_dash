...
export const useThemeConfig = () => {
  ...
  const syncRtlWithRtlLang = (rtlLangs, rtlDefaultLocale, ltrDefaultLocale) => {
    const { locale } = useI18n({ useScope: "global" });
    watch(isAppRtl, (val) => {
      if (val) locale.value = rtlDefaultLocale;
      else locale.value = ltrDefaultLocale;
    });
    watch(locale, (val) => {
      if (rtlLangs.includes(val)) isAppRtl.value = true;
    ...
    watch(
      ...
      ([valIsAppRTL, valLocale], [oldValIsAppRtl, oldValLocale]) => {
        ...
        if (isRtlUpdated) {
          if (valIsAppRTL) locale.value = rtlDefaultLocale;
          else locale.value = ltrDefaultLocale;
        } else {
          if (rtlLangs.includes(valLocale)) isAppRtl.value = true;
          else isAppRtl.value = false;
        }
      }
    );
